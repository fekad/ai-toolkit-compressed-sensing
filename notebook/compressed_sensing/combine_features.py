...
def _my_power_2(row):
    return pow(row.iloc[0], 2)

def _my_power_3(row):
    return pow(row.iloc[0], 3)

def _my_power_m1(row):
    return pow(row.iloc[0],-1)
...
def _my_abs_sqrt(row):
    return math.sqrtabs(abs(row.iloc[0]))

def _my_exp(row):
    return exp(row.iloc[0])

def _my_exp_power_2(row):
    return exp(pow(row.iloc[0], 2))

def _my_exp_power_3(row):
    return exp(pow(row.iloc[0], 3))

def _my_sum(row):
    return row.iloc[0] + row.iloc[1]

def _my_abs_sum(row):
    return abs(row.iloc[0] + row.iloc[1])
...
def _my_sum_power_2(row):
    return pow((row.iloc[0] + row.iloc[1]), 2)

def _my_sum_power_3(row):
    return pow((row.iloc[0] + row.iloc[1]), 3)

def _my_sum_exp(row):
    return exp(row.iloc[0] + row.iloc[1])

def _my_sum_exp_power_2(row):
    return exp(pow(row.iloc[0] + row.iloc[1], 2))

def _my_sum_exp_power_3(row):
    return exp(pow(row.iloc[0] + row.iloc[1], 3))
